...
export class PositionController {
    ...
    async createPosition(@Param("departmentId") departmentId: string, @Body() position: NewPositionDto) {
    ...
    @UseGuards(JwtAuthGuard)
    @Roles(Role.SUPER_ADMIN, Role.ADMIN)
    async updatePosition(@Param("id") id: string, @Body() position: NewPositionDto) {
    ...
    @UseGuards(JwtAuthGuard)
    @Roles(Role.SUPER_ADMIN, Role.ADMIN)
    async getOrganizationPositions(@CurrentUser("organizationId") organizationId: string) {
